...
"""
Introduction:
    Element Interactive Manifold Combine score global surface area
...
class EIM_Combine_Score_Global_Surface():
    ...
    def get_features(self):
        ...
        pair_wise_features = np.zeros(
            (ligand_len*protein_len, num_pairwise_features))
        ...
        for i, l_atom_type in enumerate(self.ligand_atom_type):
            ...
            for j, p_atom_type in enumerate(self.protein_atom_type):
                ...
                if np.size(ei_xyz) != 0:
                    ...
                    isovalue_area_features = np.zeros(len(self.isovalue_list))
                    isovalue_volume_features = np.zeros(len(self.isovalue_list))
                    ...
                    for isovalue_count, isovalue in enumerate(self.isovalue_list):
                        ...
                        isovalue_area_features[isovalue_count] = eisa
                        isovalue_volume_features[isovalue_count] = eisv

                    isovalue_area_stat_measure = self.get_stats(isovalue_area_features)
                    isovalue_volume_stat_measure = self.get_stats(isovalue_volume_features)
                    ...
                    curvature_features = np.concatenate([
                                                        self.get_stats(curvature_df['H'].values),
                                                        self.get_stats(curvature_df['K'].values),
                                                        self.get_stats(curvature_df['kappa_min'].values),
                                                        self.get_stats(curvature_df['kappa_max'].values),
                                                    ])

                    combine_features = np.concatenate([isovalue_area_stat_measure,
                                                       isovalue_volume_stat_measure,
                                                       curvature_features])
