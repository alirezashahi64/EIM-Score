...
"""
Introduction:
    Element Interactive Manifold Combine score local surface area
...
class EIM_Combine_Score_Local_Surface():
    def __init__(self, path, pdbid, kernel_type='exponential',
        ...
        # Build symbolic kernel function of type kernel_type for curvature calculation
    ...
    def get_features(self):
        ...
        num_pairwise_features = 6*num_stat_measure # surface area + volume + 4 types curvatures = 6


        num_features = protein_len*ligand_len*num_pairwise_features
        ...
        # Main loop
        pair_wise_features = np.zeros((ligand_len*protein_len, num_pairwise_features))
            ...
            for j, p_atom_type in enumerate(self.protein_atom_type):
                ...
                if np.size(l_xyz) != 0:
                    ...
                    for l_atom_index in range(len(l_xyz)):
                        ...
                        dmat_within_l_atom_index = cdist(
                            l_xyz_atom_index, p_xyz, metric='euclidean')
                        ...
                        atomic_surface_volume.append(eisv)

                        # Curvature Calculation
                        eic = ElementInteractiveCurvature(self.kernel, 
                                                          ligand_vdW=ligand_vdW,
                                                          protein_vdW=protein_vdW)
                        curvature_df = eic.evaluate_all_vectorized(l_xyz,
                                                                   p_xyz_within_the_ball,
                                                                   tau_val=self.kernel_tau,
                                                                   kappa_val=self.kernel_power)
                        curvature_dfs.append(curvature_df)
                        
                    # atomic_surface_stat_measure = np.zeros(num_stat_measure)

                    atomic_surface_area_stat_measure = self.get_stats(atomic_surface_area)
                    atomic_surface_volume_stat_measure = self.get_stats(atomic_surface_volume)

                    all_curvature_df = pd.concat(curvature_dfs, axis=0)
                    atomic_curvature_features = np.concatenate([
                                                    self.get_stats(all_curvature_df['H'].values),
                                                    self.get_stats(all_curvature_df['K'].values),
                                                    self.get_stats(all_curvature_df['kappa_min'].values),
                                                    self.get_stats(all_curvature_df['kappa_max'].values),
                                                ])
                    
                    combine_features = np.concatenate([atomic_surface_area_stat_measure,
                                                       atomic_surface_volume_stat_measure,
                                                       atomic_curvature_features])
