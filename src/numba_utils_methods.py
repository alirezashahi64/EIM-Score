...
@jit(nopython=True)
def surface_area_and_volume(nx, ny, nz, N_x, N_y, N_z, x, y, z, h, f, isovalue):

    area = 0.0
    vol_irregular_grid = 0.0
    vol_interior_grid = 0.0
    ...
    for i in range(nx-3):
        for j in range(ny-3):
            for k in range(nz-3):
                ...
                vol_interior_grid += h**3
                # count +=1

    volume = vol_interior_grid + 0.5*vol_irregular_grid
